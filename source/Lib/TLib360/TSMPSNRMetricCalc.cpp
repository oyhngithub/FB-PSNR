...
Void TSMPSNRMetric::sphSampoints(const std::string &cSphDataFile)
{
	...
	if (fscanf(fp, "%d", &featureNumbers) != 1)
	{
	...
	m_pCart3D = (CPos3D*)malloc(sizeof(CPos3D)*(featureNumbers));
	memset(m_pCart3D, 0, (sizeof(CPos3D) * featureNumbers));
	for (Int z = 0; z < featureNumbers; z++)
	{
