/**
 * @file 
 * @brief Replaced by E2E-PSNR-Pro
 * @author Gl.M
 * Doc:
 */
	...
	for (Int chan = 0; chan < pcPicD->getNumberValidComponents(); chan++)
	{
		...
		for (Int np = 0; np < iNumPoints; np++)
		{
			...
			if (m_codingGeoType == SVIDEO_EQUIRECT || m_codingGeoType == SVIDEO_NEWUNIFORMMAP)
			{
				if (!chan)
				{
					wsWeight = m_fErpWeight_Y[y];
				}
				else
				{
					wsWeight = m_fErpWeight_C[y];
				}
			}
				...
				)
			{
				if (!chan)
				{
					...
					{
						wsWeight = m_fCubeWeight_Y[(m_iCodingFaceWidth) * (y % (m_iCodingFaceHeight)) + (x % (m_iCodingFaceWidth))];
					}
				...
				else
				{
					...
					{
						wsWeight = m_fCubeWeight_C[(m_iCodingFaceWidth >> (pcPicD->getComponentScaleX(COMPONENT_Cb)))* (y % (m_iCodingFaceHeight >> (pcPicD->getComponentScaleY(COMPONENT_Cb)))) + (x % (m_iCodingFaceWidth >> (pcPicD->getComponentScaleX(COMPONENT_Cb))))];
					}
				}
			}
...
#endif
			{
				if (!chan)
				{
					wsWeight = m_fEapWeight_Y[y * iWidth + x];
				}
				else
				{
					wsWeight = m_fEapWeight_C[y * iWidth + x];
				}
			}
			else if (m_codingGeoType == SVIDEO_OCTAHEDRON)
			{
				if (!chan)
				{
					wsWeight = m_fOctaWeight_Y[iWidth * y + x];
				}
				else
				{
					wsWeight = m_fOctaWeight_C[y * iWidth + x];
				}
			}
			else if (m_codingGeoType == SVIDEO_ICOSAHEDRON)
			{
				if (!chan)
				{
					wsWeight = m_fIcoWeight_Y[iWidth * y + x];
				}
				else
				{
					wsWeight = m_fIcoWeight_C[y * iWidth + x];
				}
			}
			...
			else if (m_codingGeoType == SVIDEO_SEGMENTEDSPHERE)
			{
				if (!chan)
				{
					wsWeight = m_fSspWeight_Y[iWidth * y + x];
				}
				else
				{
					wsWeight = m_fSspWeight_C[y * iWidth + x];
				}
			}
#endif
			...
			else if (m_codingGeoType == SVIDEO_ROTATEDSPHERE)
			{
				if (!chan)
				{
					wsWeight = m_fRspWeight_Y[iWidth * y + x];
				}
				else
				{
					wsWeight = m_fRspWeight_C[y * iWidth + x];
				}
			}
#endif
			...
			else if (m_codingGeoType == SVIDEO_EQUIRECT && m_bPERP)
			{
				...
				if ((x < (SVIDEO_ERP_PAD_L >> getComponentScaleX(ch, fmt))) || (x >= (iWidth - (SVIDEO_ERP_PAD_R >> getComponentScaleX(ch, fmt)))))
					...
					wsWeight = (!chan) ? m_fErpWeight_Y[y] : m_fErpWeight_C[y];
#else
				if (!chan)
				{
...
#endif		
			assert(wsWeight >= 0 && wsWeight <= 1);
			trueWeight *= wsWeight;
			
			// read w2	
			weightSum += trueWeight;
			assert(iDiff <= 255);
			SSDwpsnr += iDiff * iDiff * trueWeight;
		}
		const Int maxval = 255 << (iBitDepthForPSNRCalc[toChannelType(ch)] - 8);
		m_dSMPSNRV[ch] = (SSDwpsnr ? 10.0 * log10((maxval * maxval * weightSum) / (Double)SSDwpsnr) : 999.99);
	}
